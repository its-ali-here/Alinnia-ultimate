...
export async function POST(req: Request) {
  try {
    const { userId, fullName, organizationType, organizationCode, organizationData, userData } = await req.json()

    if (!userId || !fullName || !userData?.email) {
      return NextResponse.json({ error: "Missing user information" }, { status: 400 })
    }

    // Create the user profile
    await createProfile(userId, fullName, userData.email)
